      ...
      try {
        ...
        if (!notifyUser.email && userId) {
          try {
            ...
            if (candidate) {
              notifyUser.email = notifyUser.email || candidate.email;
              notifyUser.name = notifyUser.name || candidate.name || candidate.fullName;
            }
          } catch (err) {
        ...
        if (!notifyUser.email) {
          console.warn('⚠️ No email address found for payment notification', {
            userId,
